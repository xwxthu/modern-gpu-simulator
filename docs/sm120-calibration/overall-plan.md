...
S0 through S6 are complete. S7 smoke bring-up is in progress. CUDA 13 runtime ABI, trace-driven runtime linking, runtime trace option registration, CUDA 13.1 `ptxas` parsing, runtime opcode-latency option lifetime, remodeled per-SM stats initialization, PTX-mode remodeled fetch PC/function-id handling, PTX-mode predicate-latency trace metadata guarding, PTX-mode memory-latency handling, PTX-mode scoreboarding/register-file trace metadata separation, PTX-mode memory access address setup, PTX-mode IBuffer instruction ownership, PTX-mode LD/ST trace metadata separation, PTX-mode parameter constant routing, and PTX-mode post-execution memory-latency regeneration have been fixed for the observed local smoke. The current precise blocker is a PTX-mode IBuffer PC mismatch assertion in `Subcore::single_decode()`: `ibuffer_entry.m_inst->pc == ibuffer_entry.m_pc` fails at `subcore.cc:934`, with focused evidence showing `sm_warp_id = 32`, `ibuffer_entry.m_pc = 48`, and `ibuffer_entry.m_valid = true`; real simulator metrics and real supplied-metrics correlation are still pending.
