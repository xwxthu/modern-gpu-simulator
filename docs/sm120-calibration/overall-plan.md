...
S0 through S6 are complete. S7 smoke bring-up is in progress. CUDA 13 runtime ABI, trace-driven runtime linking, runtime trace option registration, CUDA 13.1 `ptxas` parsing, runtime opcode-latency option lifetime, remodeled per-SM stats initialization, PTX-mode remodeled fetch PC/function-id handling, PTX-mode predicate-latency trace metadata guarding, PTX-mode memory-latency handling, PTX-mode scoreboarding/register-file trace metadata separation, PTX-mode memory access address setup, PTX-mode IBuffer instruction ownership, PTX-mode LD/ST trace metadata separation, PTX-mode parameter constant routing, PTX-mode post-execution memory-latency regeneration, and PTX-mode variable-size IBuffer fetch/decode have been fixed for the observed local smoke. The current precise blocker is PTX ALU pipeline classification: `Subcore::get_fu()` aborts with `ERROR. EXECUTION PIPELINE FOR THIS INSTRUCTION NOT IMPLEMENTED` for a decoded PTX `ALU_OP` / `INT__OP` instruction whose `op_pipe` remains `UNKOWN_OP`; real simulator metrics and real supplied-metrics correlation are still pending.
