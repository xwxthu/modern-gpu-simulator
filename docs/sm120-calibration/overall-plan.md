...
S0 through S6 are complete. S7 smoke bring-up is in progress. CUDA 13 runtime ABI, trace-driven runtime linking, runtime trace option registration, CUDA 13.1 `ptxas` parsing, runtime opcode-latency option lifetime, remodeled per-SM stats initialization, PTX-mode remodeled fetch PC/function-id handling, PTX-mode predicate-latency trace metadata guarding, PTX-mode memory-latency handling, PTX-mode scoreboarding/register-file trace metadata separation, and PTX-mode memory access address setup have been fixed for the observed local smoke. The current precise blocker is a PTX-mode `Scoreboard::reserveRegister()` abort at `scoreboard.cc:95`: `Subcore::issue_warp()` is issuing `sm_warp_id = 12` while the instruction carries `m_warp_id = 5` and `m_dynamic_warp_id = 5` during a reissue path; real simulator metrics and real supplied-metrics correlation are still pending.
